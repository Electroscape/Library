...
public:
    ...
    */
    ...
    bool readPassiveTargetID(uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout = 100);  // original: timeout = 1000
    ...
    // FeliCa Functions
    int8_t felica_Polling(uint16_t systemCode, uint8_t requestCode, uint8_t *idm, uint8_t *pmm, uint16_t *systemCodeResponse, uint16_t timeout=1000);   // original: timeout = 1000
    ...
    static void PrintHexCharShort(const uint8_t *data, const uint32_t numBytes);
